...
class FenPrincipale(Tk):
    def __init__(self):
        ...
        self.__boutonEffacer = Button(self, text='undo last node', command=self.undo_last_noeud).pack(side=LEFT, padx=5,
                                                                                                      pady=5)
...
class Balle:
    def __init__(self, canvas, cx, cy, rayon, couleur, fill_color="white"):
        ...
        self.__canid = self.__can.create_oval(cx - rayon, cy - rayon, cx + rayon, cy + rayon, outline=couleur,
                                              fill=fill_color)
    ...
    def NoNeed_Here_not_used_generer_un_point_XY_dans_une_bande(self, last_x, last_y):
        ...
        delta_x_centre = random.randint(10, 50);
        delta_y_centre = random.randint(10, 50)
        hasard_x = random.randint(0, 1);
        hasard_y = random.randint(0, 1)
